...
uses the existing PostGIS repository in a transaction. ADM2–ADM5 are only
published through a separate, checksummed, identity-bound operator parent-map
review; no spatial or name inference exists. Automatic refresh, source
precedence, and authoritative overrides remain deferred.
...
| Next.js 16.3 + React 19.2 **current**                                                                                          | Marketing client, same-origin API bridge and first authenticated app shell       | Browser UI unavailable; API may remain healthy                | Built assets/config; source in git                                                                                                                                                                                                                                                                       | Through Caddy/Next routing; stateless replicas                       | MIT; HTTP API boundary permits another web client                                                                       |
...
| Prisma 7.9.1 **current**                                                                                                       | Relational adapter and generated client                                          | DB operations fail                                            | Schema and migrations through organization/RLS (`20260823204922_init`, `20260824000000_organizations_rls`, `20260824103000_fix_organization_bootstrap_policy`, `20260824104000_relax_rls_setting_helpers`, `20260824110000_harden_invitation_rls`) are in git and deployed locally against real Postgres | Private library; scale with process and pool                         | Apache-2.0; repository ports. Prisma 8 RC is **deferred** until GA and a migration prompt                               |
...
- **Current** `accounts`, `auth`, and `sessions` become the identity boundary;
  their opaque hashed database sessions remain the starting mechanism. Account
  token persistence for recovery/verification is current, without a public
  mail-dependent route.
- **Current** `organizations` owns `Organization`, `Membership`, `Invitation`,
  fixed role permissions, active organization context, audit append and forced
  RLS for its tenant tables.
...
| Tenancy **current for organization admin** | `Organization`; unique membership `(organization_id, account_id)`; invitation token hash, normalized email, role, expiry, accepted/revoked state; optional entitlement without billing provider remains target                                | Last owner protected by trigger; revoke membership rather than silently cascading authored history; org deletion policy open                | account organizations; org members by role/status; invitation hash/email; org-scoped permission checks                             |
...
- Global geography has an adjacency-list hierarchy, source/provenance records,
  deterministic source codes, aliases, and a PostGIS `RegionGeometry` table.
- Tenant datasets, mappings, ingestion runs, validation issues, staged source
  summaries, and immutable dataset versions exist in PostgreSQL with forced RLS.
  Composite tenant foreign keys prevent cross-organization references between
  ingestion rows and their datasets, uploads, mappings, runs, and versions.
- CSV and XLSX parser adapters use verified MIT packages; GeoJSON inspection is
  local and bounded. Parser output is a compact summary plus issues, not raw
  uploaded rows stored in PostgreSQL; mapped region validation checks every
  parser-accepted validation row, not just preview samples.
- The API exposes `/api/v1/datasets` and `/api/v1/ingestion-runs` routes behind
