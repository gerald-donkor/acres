...
| --- | --- | --- | --- | --- | --- |
| Node 22 **current**; Node 24 LTS **target** | JS runtime for build, API, worker, Next | Affected process unavailable | None; reproducible image | Private process; scale API/worker separately | MIT terms for Node-maintained code plus bundled third-party licenses; OCI/runtime seam. Upgrade only after package and Prisma-generator verification |
...
| Prisma 7.9.1 **current** | Relational adapter and generated client | DB operations fail | Schema is in git; migrations are **target** phase 2 | Private library; scale with process and pool | Apache-2.0; repository ports. Prisma 8 RC is **deferred** until GA and a migration prompt |
| PostgreSQL + PostGIS **target** | Authoritative tenant, lineage, analytics, job/outbox, audit, spatial state | Product data unavailable | Required PITR/base backups; restore tests | Private; primary/replica or partition only from measured need | PostgreSQL License + GPL-2.0-or-later PostGIS; SQL/repository boundary |
