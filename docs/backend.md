...
### GraphQL cursor codec, pagination, loaders, error filter, resolver, and regional predicates

The presentation-tier GraphQL subsystem and regional shared types export canonical cursor kinds, type predicates, and isolated unit test suites (prompt 152):
- `packages/shared/src/regions.ts` exports canonical type predicate `isInsightReportStatus` over `INSIGHT_REPORT_STATUSES`.
- `server/src/graphql/cursor-codec.ts` exports canonical const tuple `CURSOR_KINDS = ['organizationMembers', 'organizationInvitations', 'organizationAuditEvents', 'regions'] as const`, derived union type `CursorKind`, and type predicate `isCursorKind`.
- `server/src/graphql/cursor-codec.spec.ts` (19 tests) verifies `CursorCodec`: HMAC generation using `sessionSecret` and salt `acres.cursor.v1`, base64url encoding, `timingSafeEqual` constant-time verification, rejection of tampered HMAC, malformed delimiters, invalid JSON payload, invalid version (`v !== 1`), mismatched cursor kind, mismatched organization ID, invalid sort array, and `isCursorKind` predicate validation.
- `server/src/graphql/pagination.spec.ts` (26 tests) verifies GraphQL pagination helpers: `connectionWindow` default first sizing (bounded by `graphqlMaxFirst`), bounds enforcement (`first < 1`, `first > graphqlMaxFirst`, non-integer checks throwing `ApiException.queryLimitExceeded`), cursor decoding with kind and tenant ID, `connectionFromWindow` edge building, `hasNextPage` determination, `endCursor` identification, and sort field fallback (`createdAt` -> `name` -> empty string).
- `server/src/graphql/graphql-error.filter.spec.ts` (17 tests) verifies `GraphqlErrorFilter`: `ApiException` unwrapping to `GraphQLError` with status code in extensions, `HttpException` custom body unwrapping, unhandled exception masking to generic `INTERNAL_ERROR` with stack logging, request ID propagation from GraphQL context, and fallback behavior when context is absent.
- `server/src/graphql/graphql.loaders.spec.ts` (8 tests) verifies `createGraphqlLoaders`: DataLoader batching behavior, `regionBySlug.load` delegating to `RegionsService.findBySlugs`, mapping of found entries, generation of `ApiException.notFound` for missing slugs, partial batch error isolation, per-loader query caching, and preservation of requested slug ordering.
...
### Shared predicates, GraphQL limits, and decorator unit testing

The shared contracts, GraphQL query budgeting plugin, context decorators, and security throttlers export canonical tuples, predicates, and isolated unit test suites (prompt 155):
- `packages/shared/src/reports.ts` exports canonical const tuple `AI_GENERATION_STATES = ['succeeded', 'validation_rejected', 'rate_limited', 'timeout', 'unavailable', 'malformed_output', 'grounding_rejected', 'failed'] as const`, derived type `AiGenerationState`, and type predicate `isAiGenerationState`.
- `packages/shared/src/uploads.ts` exports type predicates `isStoredObjectState` and `isUploadState`.
- `packages/shared/src/ingestion.ts` exports type predicate `isIngestionRunState`.
- `packages/shared/src/organizations.ts` exports type predicates `isOrganizationPermission` and `isAuditAction`.
- `server/src/contracts/shared-predicates.spec.ts` (34 tests) verifies all canonical predicates and constants exported across `@acres/shared` (`isApiErrorCode`, `isNodeEnv`, `isApiError`, `isAccountTokenPurpose`, `isJobRunStatus`, `isScheduledJobName`, `isOrganizationRole`, `isInvitationRole`, `isOrganizationPermission`, `isAuditAction`, `isStoredObjectState`, `isUploadState`, `isTerminalUploadState`, `isIngestionRunState`, `isTerminalIngestionRunState`, `isIngestionRunStage`, `isDatasetState`, `isMappingValidationStatus`, `isValidationIssueSeverity`, `isReportStatus`, `isReportRevisionStatus`, `isReportEvidenceType`, `isExportFormat`, `isExportStatus`, `isTerminalExportStatus`, `isAiGenerationState`, `isMetricValueKind`, `isMetricAggregationType`, `isDashboardPresentationChart`, `isDashboardCompareBy`, `isDashboardViewStatus`, `isMetricDefinitionStatus`, `isInsightReportStatus`, `VALIDATION`, `DEFAULT_CONTACT_SOURCE`, `CSRF_HEADER_NAME`, `CSRF_ERROR_CODE`, `CSRF_ERROR_MESSAGE`, `ORGANIZATION_HEADER_NAME`, `REQUEST_ID_HEADER_NAME`, `IDEMPOTENCY_HEADER_NAME`, `ANONYMOUS_SESSION`).
- `server/src/graphql/graphql-limits.ts` exports `countOperation`, `CONNECTION_FIELDS`, and `isConnectionField`.
- `server/src/graphql/graphql-limits.spec.ts` (20 tests) verifies GraphQL query budget and complexity plugin (`createGraphqlLimitPlugin`) and AST analyzer (`countOperation`): requires `operationName` in test/production, allows omission in development, enforces single operation per document, enforces `graphqlMaxAliases`, `graphqlMaxDepth`, `graphqlMaxNodes`, and `graphqlMaxCost`, and handles literal/variable `first` args, fragment spreads, inline fragments, and connection field defaults.
- `server/src/organizations/current-organization.decorator.spec.ts` (3 tests) verifies `getOrganizationFromContext` and `CurrentOrganization`: extracts resolved context, throws Error when unresolved.
