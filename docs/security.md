...
| TM-02 | Role escalation, invitation replay, last-owner removal         | Medium / High            | Central permission map, non-owner generic role assignment, expiring hash-only invites, audit, and forced-RLS last-owner trigger are implemented. Prompt 55's real PostgreSQL gate concurrently transfers ownership to competing targets and proves one active owner/audit/idempotency outcome; it also proves direct sole-owner demotion is rejected and rolled back. The organization lock precedes in-transaction permission evaluation, preventing a waiter from using a stale owner role.                                                              | Broaden lifecycle fixtures as membership workflows grow; invitation delivery/recovery and distributed operational policy remain later work                                                                                                                                        | Phase 3 follow-up/5 |
