...
| TM-18 | Dependency/action/container supply-chain compromise            | Medium / Critical        | npm lockfile/`npm ci`, CI `contents: read`, non-root image. Prompt 63 implements `scripts/ops/run-deployment-drill.sh` verifying non-destructive additive migrations, graceful shutdown drain (`stop_grace_period`: Caddy 30s, Next 30s, API 45s, Worker 60s), and parameterizable image rollback. Prompt 65 implements deterministic CycloneDX v1.5 JSON SBOM generation with license compliance verification (`scripts/ops/generate-sbom.js`), pure Node.js SAST scanning (`scripts/ops/run-sast-scan.js`) with fail-closed triage policy (`infra/security/sast-triage.json`), and static container image/Compose hardening verification (`scripts/ops/verify-container-security.js`). Prompt 163 requires client/server digest references in production Compose and validates their syntax and shared API/worker wiring before release. Prompt 164 adds a fail-closed release record binding reviewed commit, current and rollback pairs, and distinct evidence references. Prompt 165 makes every approved CLI launch check compare the recorded current pair with the exported client/server pair, including the standard aggregate command. This is record consistency only. | Cryptographic provenance verification, publication, protected promotion, and live operator approval remain operator-owned and open. | Phase 12 |
...
- **Fail-Closed Execution**: Running `npm run ops:launch-readiness` executes baseline template, secret, and runtime checks, then validates the readiness record. The checked-in template intentionally fails with 69 blockers, preventing unauthorized launch approval until real operator decisions and live drills are conducted.
...
- **Prometheus Alerting Rule Expansion & Simulation Engine (TM-16, TM-20)**:
  ...
  - **2026-09-23 correction:** Prompt 66's seventh alert name implied database
